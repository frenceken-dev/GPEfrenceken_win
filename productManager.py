...
class ProductoManager:
    ...
    def __init__(self, root, imagen_panel_tk, volver_menu):
        ...
        self.empaques_seleccionados = [] # Kit de empaque
        self.items_kit_empaque = [] # Empaques que conforman el kit
    ...
    def crear_producto(self):
        ...
        codigos, self.items_kit_empaque, self.costo_kit_empaque = db_connect.selecion_kit_empaques()  # ["KIT-1", "KIT-2", ...]
        self.combobox_empaque['values'] = codigos
        ...
        # Label para mostrar la descripción del empaque seleccionado
        self.descripcion_empaque = tk.Label(
            self.frame_empaques,
            text="", #"Selecciona un empaque para ver su descripción.",
            wraplength=500,
            bg="#a0b9f0",
            justify="left"
        )
        self.descripcion_empaque.pack(fill=tk.X, pady=5)

        # Vincular el evento de selección del Listbox
        self.listbox_empaques.bind("<<ListboxSelect>>", self.mostrar_descripcion_empaque)
        ...
        crear_boton(
            ...
            font=("Arial", 11, "bold"),
            hover_color="#2ECC71",
            comando=lambda: self.guardar_borrador(self.form_frame)
        ).grid(row=13, column=0, columnspan=2, pady=10)
        
    
    # Función para mostrar la descripción del empaque seleccionado
    def mostrar_descripcion_empaque(self, event):
        seleccion = self.listbox_empaques.get(self.listbox_empaques.curselection())
        ...
        self.descripcion_empaque.config(text=descripcion)
    ...
    def eliminar_empaque_de_lista(self):
        ...
        if seleccion:
            ...
            self.descripcion_empaque.configure(text="")
    ...
    def actualizar_color(self, event, color_entry):
        ...
        if codigo_material:
            color = db_connect.obtener_color_por_material(codigo_material)
    ...
    def actualizar_tipos(self, event, tipo_entry, material_entry, color_entry):
        ...
        if codigo_material and color_material:
            tipos = db_connect.obtener_tipos_por_material_y_color(codigo_material, color_material)
    ...
    def actualizar_tamaños(self, event, tamaño_entry, material_entry, color_entry, tipo_entry):
        ...
        if codigo_material and color_material and tipo_material:
            tamaños = db_connect.obtener_tamaños_por_material_color_tipo(codigo_material, color_material, tipo_material)
    ...
    def guardar_borrador(self, material_window):
        ...
        # Empaques
        materiales_de_empaque = self.empaques_seleccionados.copy()  # Lista de los kit de empaques        
        ...
        borrador_existente = db_connect.existe_borrador(codigo_producto)
        ...
        if borrador_existente is not None:
            db_connect.actualizar_borrador(id_creador, nombre_creador, codigo_producto, tipo_producto,
    ...
    def mostrar_borradores_pendientes(self):
        ...
        if borradores:  # Posible solucion productManager.py", line 658, in mostrar_borradores_pendientes for i, borrador in enumerate(borradores): TypeError: 'NoneType' object is not iterable
